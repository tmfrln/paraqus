...
with CollectionWriter(vtu_writer, collection_name) as collection_writer:
    ...
    for time in np.linspace(0, tmax, 21):
        ...
        # scale the displacement field and add the scaled field to the model
        scale = time/tmax
...
# if you open the .pvd file in paraview, the .vtu files for the individual
# models are interpreted as a time series and can be played as a video to
# visualize how the deformation changes over time