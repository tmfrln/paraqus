...
databases. Datails on this example are available in the Abaqus Example
Problems Guide:

    Example Problems -> Static Stress/Displacement Analysis -> Forming
    analyses -> Upsetting of a cylindrical billet: quasi-static analysis
    with mesh-to-mesh solution mapping (Abaqus/Standard) and adaptive
...
leaving us with two ODBs containing results for different points of
time.

After the files 'billet_case1_std_coarse.odb' and
'billet_case1_std_coarse_rez.odb' have been created, run this script in
...
# You will loop over these, each entry corresponds to one output
# database
STEP_NAMES = ["Step-1", "Step-1"]  # Name of the step to be exported
FRAME_INDICES = [(0, -1), (1, -1)]  # Frame indices to export per ODB

# Create a writer that will write the exported results to a .vtu file.
# Use the BinaryWriter this time, creating more efficient but not
...
# time steps. This allows you to combine multiple files, representing
# different parts of the model at different times, into one
# representation in ParaView. It also makes it possible to export videos
# based on time instead of j t a sequence (useful if time steps are not
# spaced equally).
with CollectionWriter(vtu_writer, "Compression Test") as coll_writer:
    ...
    for step, frames, odb in zip(STEP_NAMES, FRAME_INDICES, ODB_PATHS):
        # Create a new reader for each ODB
        ...
        # Always export the same results
